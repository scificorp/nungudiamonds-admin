...
import { DELIVERY_STATUS, GIFTSET_DELIVERY_STATUS, GIFTSET_ORDER_STATUS_UPDATE, GIFTSET_ORDERS_DETAIL, ORDER_STATUS_UPDATE, ORDERS_DETAIL } from "src/services/AdminServices"
...
const OrderDetails = () => {
    ...
    const [orderDetailData, setOrderDetailData] = useState<{ id: number, quantity: number, sub_total: number, product_tax: any, product_name: string, product_sku: string, product_images: any[], product_price: any, delivery_status: any, discount: any }[]>([])
    ...
    const invoiceOnClick = () => {
        Router.push({ pathname: "/orders/invoice/giftset-invoice-list", query: { id: orderData.id } })
    }

    const invoicePrintOnClick = () => {
        Router.push({ pathname: "/orders/invoice/giftset-invoice-layout", query: { id: orderData.id } })
    }
    ...
    const ordersDetailsData = async (orderNumber: any) => {
        ...
        try {
            const data = await GIFTSET_ORDERS_DETAIL(payload);
            ...
            if (data.code === 200 || data.code === "200") {
                ...
                for (const item of data.data.gift_order) {
                            ...
                            {orderDetailData.map((details, k) => (
                                <Grid item lg={6} md={6} xs={12} key={details.id}>
                                    <Grid item xs={12}>
                                        <Card>
                                            <CardContent>
                                                <Typography><b>{details.product_name}</b></Typography>
                                                <Typography>{details.price}</Typography>
                                            </CardContent>
                                            <Divider />
                                            <CardContent>
                                                <Typography><b>Diamond Price: </b>{details.diamond_rate}</Typography>
                                                <Typography><b>Gold KT: </b>{details.Karat}</Typography>
                                                <Typography><b> Metal tone: </b>{details.metal}</Typography>
                                                <Typography><b>Metal Tone: </b>{details.Metal_tone}</Typography>
                                                <Typography><b>Size: </b>{details.product_size}</Typography>
                                                <Typography><b>length: </b>{details.product_length}</Typography>
                                            </CardContent>
                                        </Card>
                                    </Grid>
                    ...
                    <Grid item xs={12}>
                        ...
                        <Card sx={{ mt: 6 }} >
                            ...
                            <CardContent sx={{ display: 'flex', alignItems: 'center' }}>
                                ...
                                <Typography sx={{ fontSize: "13px" }}>{orderData.user_name ? orderData.user_name : shippingData.full_name}</Typography>
                            </CardContent>
                            ...
                            <CardContent>
                                ...
                                <Typography sx={{ mt: 4 }}><b>Name:</b> {shippingData.full_name}</Typography>
                            ...
                            <CardContent>
                                ...
                                <Typography sx={{ mt: 4, fontSize: "13px" }}><b>Name:</b> {billingData.full_name}</Typography>
