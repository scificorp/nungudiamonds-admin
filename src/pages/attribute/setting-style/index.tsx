...
import { SETTING_STYLE_ADD, SETTING_STYLE_DELETE, SETTING_STYLE_EDIT, SETTING_STYLE_GET_ALL, SETTING_STYLE_STATUS } from 'src/services/AdminServices'
...
const SettingStyle = () => {
  ...
  const [settingStyleSlug, setSettingStyleSlug] = useState('')
  ...
  const defaultValues = {
    ...
    settingStyleSlug: settingStyleSlug,
    sortCode: sortCode,
  }
  const {
    control,
  ...
  const editOnClickHandler = async (data: any) => {
    ...
    setValue("settingStyleSlug", data.slug);
  ...
  const SettingStyleAddApi = async (data: any) => {
    ...
    formData.append("slug", data.settingStyleSlug)
  ...
  const handleChangePerPageRows = (perPageRows: number) => {
    SettingStyleGetAllApi({ ...settingStylePagination, per_page_rows: perPageRows, current_page: 1 })
  }

  const handleOnPageChange = (page: number) => {
    SettingStyleGetAllApi({ ...settingStylePagination, current_page: page + 1 })
  }
  ...
  const SettingStyleEditApi = async (data: any) => {
    ...
    formData.append("sort_code", data.sortCode)
    try {
      const data = await SETTING_STYLE_EDIT(formData);
      if (data.code === 200 || data.code === "200") {
        toast.success(data.message);
        toggleAddSettingStyleDrawer();
        clearFormDataHandler();
  ...
  const SettingStyleDeleteApi = async () => {
    ...
    try {
      const data = await SETTING_STYLE_DELETE(payload);
  ...
  const SettingStyleStatusApi = async (checked: boolean, row: any) => {
    ...
    try {
      const data = await SETTING_STYLE_STATUS(payload);
              ...
              <Controller
                ...
                render={({ field }: any) => (
                  <TextField
                    autoFocus
                    size='small'
                    label='Setting Style Slug'
                    value={settingStyleSlug}
                    onChange={(e) => setSettingStyleSlug(e.target.value)}
                    error={Boolean(errors.settingStyleSlug)}
                    {...field}
                  />
                )}
              />
              {errors.settingStyleSlug && <FormHelperText sx={{ color: 'error.main' }}>{FIELD_REQUIRED}</FormHelperText>}
            </FormControl>
            <FormControl fullWidth sx={{ mb: 4 }}>
              <Controller
                name='sortCode'
